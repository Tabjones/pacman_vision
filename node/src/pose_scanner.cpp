...
bool
PoseScanner::computeTableTransform(PT pt, float nx, float ny, float nz)
{
    try
    {
        ...
        /*
         * Rotation matrix of new table reference system with respect to
         * kinect ref. frame. Translation that express table ref system centre
...
bool
PoseScanner::saveTableTransform()
{
    ...
    try
    {
        if (!boost::filesystem::exists(session_dir) ||
                                !boost::filesystem::is_directory(session_dir))
        ...
        flann::save_to_file(T_kt_flann, session_dir.string()+"T_kt.h5",
                                        "TurnTable with respect to Kinect");
    }
    catch (...)
    {
        return (false);
...
void PoseScanner::cb_clicked (const geometry_msgs::PointStamped::ConstPtr& msg)
{
    if(!ignore_clicked_point)
    {
        ...
        {
            ROS_WARN("[PoseScanner][%s]\tFailed to compute TableTransform, please click again!"
                                                                    ,__func__);
            return;
        }
        if (!saveTableTransform())
            ROS_WARN("[PoseScanner][%s]\tFailed to save TableTransform to disk"
                                                                    ,__func__);
    }
}

bool
PoseScanner::cb_acquire(pacman_vision_comm::acquire::Request& req,
                                    pacman_vision_comm::acquire::Response& res)
{
    if (!has_transform)
    {
        ROS_ERROR("[PoseScanner][%s]\tNo table transform defined, please click and publish a point in rviz"
                                                                    ,__func__);
        return (false);
    }
    std::string name;
    ...
    {
        ROS_ERROR("[PoseScanner][%s]\tCannot move TurnTable correctly!"
                                                                    ,__func__);
        return false;
    }
...
bool
PoseScanner::set_turn_table_pos(float pos)
{
    ...
    {
        ROS_ERROR("[PoseScanner][%s]\tService for setting turn table position failed!"
                                                                ,__func__);
        return (false);
    }
    return (true);
...
float
PoseScanner::get_turn_table_pos()
{
    ...
    {
        ROS_ERROR("[PoseScanner][%s]\tService for getting turn table position failed!"
                                                                ,__func__);
        return (-1);
    }
...
bool
PoseScanner::move_turn_table_smoothly(float pos)
{
...
bool
PoseScanner::cb_reload(pacman_vision_comm::reload_transform::Request& req,
                        pacman_vision_comm::reload_transform::Response& res)
{
                                    ...
                                    boost::filesystem::is_regular_file(file))
    {
        try
        {
            ...
            flann::load_from_file(trans, file.string(),
                                        "TurnTable with respect to Kinect");
    ...
    {
        ROS_ERROR("[PoseScanner][%s]\tSpecified path does not name a file or does not exists!"
                                                                ,__func__);
        return (false);
    }
    return (true);
...
PoseScanner::spin_once()
{
    ...
    if (has_transform)
    {
        ...
        tf_table_trans.sendTransform(tf::StampedTransform(t_kt,
