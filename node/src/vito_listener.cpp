...
Listener::Listener(ros::NodeHandle &n, boost::shared_ptr<Storage> &stor)
{
    ...
    this->storage = stor;
    this->srv_get_cloud = nh.advertiseService("get_cloud_in_hand",
                                        &Listener::cb_get_cloud_in_hand, this);
...
Listener::listen_table()
{
    ...
    try
    {
        tf_listener.waitForTransform(sens_ref_frame.c_str(),
                    "/workbench_plate_link", ros::Time(0), ros::Duration(2.0));
        tf_listener.lookupTransform(sens_ref_frame.c_str(),
                            "/workbench_plate_link", ros::Time(0), table_tf);
    }
    catch (tf::TransformException& ex)
    {
        ROS_WARN("%s", ex.what());
        ROS_WARN("[Listener][%s] Can not find Table Transformation. Using identity..."
                                                                    ,__func__);
...
Listener::listen_once()
{
    ...
    if (listen_left_arm)
    {
        ...
        try
        {
            ...
            {
                tf_listener.waitForTransform(sens_ref_frame.c_str(),
                                    (left+arm_naming[i]).c_str(), ros::Time(0),
                                                            ros::Duration(2.0));
                tf_listener.lookupTransform(sens_ref_frame.c_str(),
                                    (left+arm_naming[i]).c_str(), ros::Time(0),
                                                                left_arm_tf[i]);
        ...
        {
            ROS_WARN("%s", ex.what());
            ROS_WARN("[Listener][%s] Can not find Vito Left Arm Transforms, disabling lookup..."
                                                                    ,__func__);
    ...
    if (listen_right_arm)
    {
        ...
        try
        {
            ...
            {
                tf_listener.waitForTransform(sens_ref_frame.c_str(),
                                    (right+arm_naming[i]).c_str(), ros::Time(0),
                                                            ros::Duration(2.0));
                tf_listener.lookupTransform(sens_ref_frame.c_str(),
                                    (right+arm_naming[i]).c_str(), ros::Time(0),
                                                                right_arm_tf[i]);
        ...
        {
            ROS_WARN("%s", ex.what());
            ROS_WARN("[Listener][%s] Can not find Vito Right Arm Transforms, disabling lookup..."
                                                                    ,__func__);
    ...
    if (listen_left_hand)
    {
        ...
        {
            tf_listener.waitForTransform(sens_ref_frame.c_str(),
                    "/left_hand_palm_link", ros::Time(0), ros::Duration(2.0));
            tf_listener.lookupTransform(sens_ref_frame.c_str(),
                            "/left_hand_palm_link", ros::Time(0), left_hand_tf);
            fromTF(left_hand_tf, *left_hand, pose);
        }
        catch (tf::TransformException& ex)
        {
            ROS_WARN("%s", ex.what());
            ROS_WARN("[Listener][%s] Can not find Vito Left Hand Transformation, disabling lookup..."
                                                                    ,__func__);
    ...
    if (listen_right_hand)
    {
        ...
        {
            tf_listener.waitForTransform(sens_ref_frame.c_str(),
                    "/right_hand_palm_link", ros::Time(0), ros::Duration(2.0));
            tf_listener.lookupTransform(sens_ref_frame.c_str(),
                        "/right_hand_palm_link", ros::Time(0), right_hand_tf);
            fromTF(right_hand_tf, *right_hand, pose);
        }
        catch (tf::TransformException& ex)
        {
            ROS_WARN("%s", ex.what());
            ROS_WARN("[Listener][%s] Can not find Vito Right Hand Transformation, disabling lookup..."
                                                                    ,__func__);
...
bool
Listener::cb_get_cloud_in_hand(pacman_vision_comm::get_cloud_in_hand::Request&
                    req, pacman_vision_comm::get_cloud_in_hand::Response& res)
{
    ...
    {
        if (i == 0)
            listen_and_extract_detailed_hand_piece(req.right, i, cloud_original,
                                                                        hand);
        else
            listen_and_extract_detailed_hand_piece(req.right, i, cloud_original,
                                                                        piece);
    ...
    if (req.save.compare("false") != 0)
    {
        ...
        if (boost::filesystem::exists(save_dir) &&
                                    boost::filesystem::is_directory(save_dir))
        {
            ...
            writer.writeASCII((save_dir.string() + "/obj.pcd").c_str(),
                                                                obj_rgba,16 );
            writer.writeASCII((save_dir.string() + "/hand.pcd").c_str(),
                                                                hand_rgba,16 );
        }
        else
        {
            ROS_ERROR ("[Listener][%s] Invalid save directory passed to service: %s"
                                                ,__func__,save_dir.c_str());
            return false;
        }
    }
...
void
Listener::listen_and_crop_detailed_hand_piece(bool right, size_t idx,
                                                                PC::Ptr& cloud)
{
    ...
    {
        tf_listener.waitForTransform(sens_ref_frame.c_str(),
                        (hand+piece).c_str(), ros::Time(0), ros::Duration(2.0));
        tf_listener.lookupTransform(sens_ref_frame.c_str(),
                                (hand+piece).c_str(), ros::Time(0), tf_piece);
    ...
    catch (tf::TransformException& ex)
    {
        ...
        ROS_WARN("[Listener][%s] Can not find %s Transformation, setting identity"
                                                ,__func__,(hand+piece).c_str());
        trans.setIdentity();
    }
    //crop it
    PC::Ptr out (new PC);
    if (right)
        crop_a_box(cloud, out, trans, soft_hand_right[idx]*box_scale, true,
                                                                        false);
    else
        crop_a_box(cloud, out, trans, soft_hand_left[idx]*box_scale, true,
                                                                        false);
    pcl::copyPointCloud(*out, *cloud);
}

void
Listener::listen_and_extract_detailed_hand_piece(bool right, size_t idx,
                                                PC::Ptr& cloud, PC::Ptr& piece)
{
    ...
    //crop it
    if (right)
        crop_a_box(cloud, piece, trans, soft_hand_right[idx]*box_scale, false,
                                                                        false);
    else
